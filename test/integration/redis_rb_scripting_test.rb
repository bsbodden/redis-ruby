...
class RedisRbScriptingTest < Minitest::Test
  ...
  def test_script_exists
    ...
    assert r.script(:exists, a)
    refute r.script(:exists, b)
  ...
  def test_eval_with_keys_and_args
    assert_equal %w[k1 k2], r.eval("return KEYS", %w[k1 k2])
    assert_equal %w[a1 a2], r.eval("return ARGV", [], %w[a1 a2])
  end

  def test_eval_with_options_hash
    assert_equal 0, r.eval("return #KEYS", {})
    assert_equal 0, r.eval("return #ARGV", {})
    assert_equal %w[k1 k2], r.eval("return KEYS", keys: %w[k1 k2])
    assert_equal %w[a1 a2], r.eval("return ARGV", argv: %w[a1 a2])
  end
  ...
  def test_evalsha_with_keys_and_args
    assert_equal %w[k1 k2], r.evalsha(to_sha("return KEYS"), %w[k1 k2])
    assert_equal %w[a1 a2], r.evalsha(to_sha("return ARGV"), [], %w[a1 a2])
  end

  def test_evalsha_with_options_hash
    assert_equal 0, r.evalsha(to_sha("return #KEYS"), {})
    assert_equal 0, r.evalsha(to_sha("return #ARGV"), {})
    assert_equal %w[k1 k2], r.evalsha(to_sha("return KEYS"), keys: %w[k1 k2])
    assert_equal %w[a1 a2], r.evalsha(to_sha("return ARGV"), argv: %w[a1 a2])
  end
