...
class StringDSLTestPart2 < RedisRubyTestCase
  use_testcontainers!
  ...
  def test_exists_returns_true_when_key_exists
    ...
    assert_predicate str, :exists?
  end

  def test_exists_returns_false_when_key_does_not_exist
    str = redis.string(@key)

    refute_predicate str, :exists?
  end
...
class StringDSLTestPart3 < RedisRubyTestCase
  use_testcontainers!
  ...
  def test_configuration_management_workflow
    ...
    api_key.set("sk_live_123456").expire(86_400)
