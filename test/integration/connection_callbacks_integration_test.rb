...
module RR
  class ConnectionCallbacksIntegrationTest < Minitest::Test
    def setup
      @events = []
    end
    ...
    def test_client_error_callback
      @client = build_invalid_client
      conn = build_invalid_connection
      conn.register_callback(:error) { |event| @events << [:error, event] }
      @client.instance_variable_set(:@connection, conn)

      assert_raises(RR::ConnectionError) { conn.send(:connect) }
    ...
    def test_multiple_callbacks_for_same_event
      ...
      @client.register_connection_callback(:disconnected) do |_event|
    ...
    def test_callback_deregistration
      ...
      callback = ->(_event) { callback_called = true }
    ...
    def test_reconnect_triggers_reconnected_event
      ...
      reconnected = false
      @client.register_connection_callback(:reconnected) do |_event|
        reconnected = true
      end
