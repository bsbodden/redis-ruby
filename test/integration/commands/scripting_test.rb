...
class ScriptingIntegrationTest < Minitest::Test
  def setup
    @redis = RedisRuby.new(host: ENV.fetch("REDIS_HOST", "redis"), port: ENV.fetch("REDIS_PORT", 6379).to_i)
    @redis.del("script:key1", "script:key2", "script:counter")
  end
  ...
  def test_eval_with_keys
    @redis.set("script:key1", "value1")
    result = @redis.eval("return redis.call('GET', KEYS[1])", 1, "script:key1")
    assert_equal "value1", result
  end

  def test_eval_with_keys_and_args
    result = @redis.eval("return redis.call('SET', KEYS[1], ARGV[1])", 1, "script:key1", "myvalue")
    assert_equal "OK", result
    assert_equal "myvalue", @redis.get("script:key1")
  end

  def test_eval_multiple_keys_and_args
    result = @redis.eval(
      "redis.call('SET', KEYS[1], ARGV[1]); redis.call('SET', KEYS[2], ARGV[2]); return 'OK'",
      2, "script:key1", "script:key2", "val1", "val2"
    )
    assert_equal "OK", result
    assert_equal "val1", @redis.get("script:key1")
    assert_equal "val2", @redis.get("script:key2")
  end

  def test_eval_redis_call
    @redis.set("script:counter", "10")
    result = @redis.eval("return redis.call('INCR', KEYS[1])", 1, "script:counter")
  ...
  def test_eval_complex_script
    ...
    result = @redis.eval(script, 1, "script:counter", "100")
  ...
  def test_evalsha_with_keys_and_args
    ...
    result = @redis.evalsha(sha, 1, "script:key1", "sha_value")
    assert_equal "OK", result
    assert_equal "sha_value", @redis.get("script:key1")
  end
  ...
  def test_eval_ro
    @redis.set("script:key1", "readonly_value")
    result = @redis.eval_ro("return redis.call('GET', KEYS[1])", 1, "script:key1")
  ...
  def test_evalsha_ro
    ...
    sha = @redis.script_load(script)
    @redis.set("script:key1", "readonly_sha")

    result = @redis.evalsha_ro(sha, 1, "script:key1")
  ...
  def test_script_flush_async
    sha = @redis.script_load("return 'async flush'")
    result = @redis.script_flush(:async)
    assert_equal "OK", result
  end

  def test_script_flush_sync
    sha = @redis.script_load("return 'sync flush'")
  ...
  def test_eval_atomic_increment
    @redis.set("script:counter", "0")

    # Run 10 increments
    script = "return redis.call('INCR', KEYS[1])"
    10.times do
      @redis.eval(script, 1, "script:counter")
    end

    assert_equal "10", @redis.get("script:counter")
  end
  ...
  def test_eval_compare_and_swap
    ...
    @redis.set("script:key1", "old_value")

    # CAS with wrong expected value
    result = @redis.eval(script, 1, "script:key1", "wrong_value", "new_value")
    assert_equal 0, result
    assert_equal "old_value", @redis.get("script:key1")

    # CAS with correct expected value
    result = @redis.eval(script, 1, "script:key1", "old_value", "new_value")
    assert_equal 1, result
    assert_equal "new_value", @redis.get("script:key1")
  end
end
