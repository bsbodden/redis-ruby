...
class RedisCompatTest < Minitest::Test
  ...
  def test_initialize_with_defaults
    ...
    assert_equal 5.0, redis.options[:timeout]
  end
  ...
  def test_initialize_with_connect_timeout
    ...
    assert_equal 10.0, redis.options[:timeout]
  ...
  def test_initialize_timeout_not_overridden_by_read_timeout
    ...
    assert_equal 3.0, redis.options[:timeout]
  end
  ...
  def test_mget_with_splat
    @mock_client.expects(:mget).with("k1", "k2").returns(["v1", "v2"])
    redis = build_redis
    assert_equal ["v1", "v2"], redis.mget("k1", "k2")
  end
  ...
  def test_expireat
    @mock_client.expects(:expireat).with("k", 1000000, nx: false, xx: false, gt: false, lt: false).returns(1)
    redis = build_redis
    assert_equal 1, redis.expireat("k", 1000000)
  end

  def test_pexpireat
    @mock_client.expects(:pexpireat).with("k", 1000000000, nx: false, xx: false, gt: false, lt: false).returns(1)
    redis = build_redis
    assert_equal 1, redis.pexpireat("k", 1000000000)
  end
  ...
  def test_pttl
    @mock_client.expects(:pttl).with("k").returns(120000)
    redis = build_redis
    assert_equal 120000, redis.pttl("k")
  end
  ...
  def test_expiretime
    @mock_client.expects(:expiretime).with("k").returns(1700000000)
    redis = build_redis
    assert_equal 1700000000, redis.expiretime("k")
  end

  def test_pexpiretime
    @mock_client.expects(:pexpiretime).with("k").returns(1700000000000)
    redis = build_redis
    assert_equal 1700000000000, redis.pexpiretime("k")
  end

  def test_keys
    @mock_client.expects(:keys).with("*").returns(["k1", "k2"])
    redis = build_redis
    assert_equal ["k1", "k2"], redis.keys("*")
  end
  ...
  def test_hsetnx_returns_true
    ...
    redis = build_redis
    assert_equal true, redis.hsetnx("h", "f", "v")
  end

  def test_hsetnx_returns_false
    @mock_client.expects(:hsetnx).with("h", "f", "v").returns(0)
    redis = build_redis
    assert_equal false, redis.hsetnx("h", "f", "v")
  end

  def test_hmget
    @mock_client.expects(:hmget).with("h", "f1", "f2").returns(["v1", "v2"])
    redis = build_redis
    assert_equal ["v1", "v2"], redis.hmget("h", "f1", "f2")
  end
  ...
  def test_hdel_array_flattened
    ...
    redis = build_redis
    assert_equal 2, redis.hdel("h", ["f1", "f2"])
  end

  def test_hexists_true
    @mock_client.expects(:hexists).with("h", "f").returns(1)
    redis = build_redis
    assert_equal true, redis.hexists("h", "f")
  end

  def test_hexists_false
    @mock_client.expects(:hexists).with("h", "f").returns(0)
    redis = build_redis
    assert_equal false, redis.hexists("h", "f")
  end

  def test_hkeys
    @mock_client.expects(:hkeys).with("h").returns(["f1", "f2"])
    redis = build_redis
    assert_equal ["f1", "f2"], redis.hkeys("h")
  end

  def test_hvals
    @mock_client.expects(:hvals).with("h").returns(["v1", "v2"])
    redis = build_redis
    assert_equal ["v1", "v2"], redis.hvals("h")
  end
  ...
  def test_hscan
    @mock_client.expects(:hscan).with("h", 0, match: nil, count: nil).returns(["0", [["f", "v"]]])
    redis = build_redis
    assert_equal ["0", [["f", "v"]]], redis.hscan("h", 0)
  end

  def test_hscan_iter
    enumerator = Enumerator.new { |y| y.yield ["f", "v"] }
  ...
  def test_hrandfield_with_count
    @mock_client.expects(:hrandfield).with("h", count: 2, withvalues: false).returns(["f1", "f2"])
    redis = build_redis
    assert_equal ["f1", "f2"], redis.hrandfield("h", 2)
  end
  ...
  def test_hrandfield_with_values_and_count
    @mock_client.expects(:hrandfield).with("h", count: 2, withvalues: true).returns([["f1", "v1"]])
    redis = build_redis
    assert_equal [["f1", "v1"]], redis.hrandfield("h", 2, with_values: true)
  end
  ...
  def test_hexpireat
    @mock_client.expects(:hexpireat).with("h", 1700000000, "f1", nx: false, xx: false, gt: false, lt: false).returns([1])
    redis = build_redis
    assert_equal [1], redis.hexpireat("h", 1700000000, "f1")
  end

  def test_hpexpireat
    @mock_client.expects(:hpexpireat).with("h", 1700000000000, "f1", nx: false, xx: false, gt: false, lt: false).returns([1])
    redis = build_redis
    assert_equal [1], redis.hpexpireat("h", 1700000000000, "f1")
  end
  ...
  def test_httl_with_array
    ...
    redis = build_redis
    assert_equal [60, 120], redis.httl("h", ["f1", "f2"])
  end
  ...
  def test_hexpiretime
    @mock_client.expects(:hexpiretime).with("h", "f1").returns([1700000000])
    redis = build_redis
    assert_equal [1700000000], redis.hexpiretime("h", "f1")
  end

  def test_hpexpiretime
    @mock_client.expects(:hpexpiretime).with("h", "f1").returns([1700000000000])
    redis = build_redis
    assert_equal [1700000000000], redis.hpexpiretime("h", "f1")
  end
  ...
  def test_hpersist_with_array
    ...
    redis = build_redis
    assert_equal [1, 1], redis.hpersist("h", ["f1", "f2"])
  end
  ...
  def test_lpush_array_flattened
    ...
    redis = build_redis
    assert_equal 2, redis.lpush("l", ["v1", "v2"])
  end

  def test_rpush
    @mock_client.expects(:rpush).with("l", "v1", "v2").returns(2)
    redis = build_redis
    assert_equal 2, redis.rpush("l", ["v1", "v2"])
  end
  ...
  def test_lpop_with_count
    @mock_client.expects(:lpop).with("l", 3).returns(["v1", "v2", "v3"])
    redis = build_redis
    assert_equal ["v1", "v2", "v3"], redis.lpop("l", 3)
  end
  ...
  def test_rpop_with_count
    @mock_client.expects(:rpop).with("l", 2).returns(["v1", "v2"])
    redis = build_redis
    assert_equal ["v1", "v2"], redis.rpop("l", 2)
  end

  def test_lrange
    @mock_client.expects(:lrange).with("l", 0, -1).returns(["a", "b"])
    redis = build_redis
    assert_equal ["a", "b"], redis.lrange("l", 0, -1)
  end
  ...
  def test_blpop
    @mock_client.expects(:blpop).with("l", timeout: 5).returns(["l", "v"])
    redis = build_redis
    assert_equal ["l", "v"], redis.blpop("l", timeout: 5)
  end

  def test_brpop
    @mock_client.expects(:brpop).with("l", timeout: 5).returns(["l", "v"])
    redis = build_redis
    assert_equal ["l", "v"], redis.brpop("l", timeout: 5)
  end
  ...
  def test_lmpop_with_modifier_right
    @mock_client.expects(:lmpop).with("l", direction: :right, count: 2).returns(["l", ["v1", "v2"]])
    redis = build_redis
    assert_equal ["l", ["v1", "v2"]], redis.lmpop("l", modifier: "RIGHT", count: 2)
  end
  ...
  def test_sadd_array_flattened
    ...
    redis = build_redis
    assert_equal 2, redis.sadd("s", ["m1", "m2"])
  end
  ...
  def test_smembers
    @mock_client.expects(:smembers).with("s").returns(["m1", "m2"])
    redis = build_redis
    assert_equal ["m1", "m2"], redis.smembers("s")
  end
  ...
  def test_spop_with_count
    @mock_client.expects(:spop).with("s", 2).returns(["m1", "m2"])
    redis = build_redis
    assert_equal ["m1", "m2"], redis.spop("s", 2)
  end
  ...
  def test_smove_true
    ...
    redis = build_redis
    assert_equal true, redis.smove("src", "dst", "m")
  end

  def test_smove_false
    @mock_client.expects(:smove).with("src", "dst", "m").returns(0)
    redis = build_redis
    assert_equal false, redis.smove("src", "dst", "m")
  end
  ...
  def test_sunion
    @mock_client.expects(:sunion).with("s1", "s2").returns(["m1", "m2"])
    redis = build_redis
    assert_equal ["m1", "m2"], redis.sunion("s1", "s2")
  end
  ...
  def test_zadd_single_member_returns_boolean
    @mock_client.expects(:zadd).with("z", 1.0, "m", nx: false, xx: false, gt: false, lt: false, ch: false, incr: false).returns(1)
    redis = build_redis
    assert_equal true, redis.zadd("z", 1.0, "m")
  end

  def test_zadd_single_member_returns_false_when_zero
    @mock_client.expects(:zadd).with("z", 1.0, "m", nx: false, xx: false, gt: false, lt: false, ch: false, incr: false).returns(0)
    redis = build_redis
    assert_equal false, redis.zadd("z", 1.0, "m")
  end

  def test_zadd_flat_array
    @mock_client.expects(:zadd).with("z", 1.0, "m1", 2.0, "m2", nx: false, xx: false, gt: false, lt: false, ch: false, incr: false).returns(2)
    redis = build_redis
  ...
  def test_zadd_with_incr_returns_float
    @mock_client.expects(:zadd).with("z", 1.0, "m", nx: false, xx: false, gt: false, lt: false, ch: false, incr: true).returns("3.5")
    redis = build_redis
    assert_equal 3.5, redis.zadd("z", 1.0, "m", incr: true)
  end
  ...
  def test_zscore
    ...
    redis = build_redis
    assert_equal 3.5, redis.zscore("z", "m")
  end
  ...
  def test_zrange_without_scores
    @mock_client.expects(:zrange).with("z", 0, -1, byscore: false, bylex: false, rev: false, limit: nil, withscores: false).returns(["m1", "m2"])
    redis = build_redis
    assert_equal ["m1", "m2"], redis.zrange("z", 0, -1)
  end

  def test_zrange_with_scores_flat_array
    @mock_client.expects(:zrange).with("z", 0, -1, byscore: false, bylex: false, rev: false, limit: nil, withscores: true).returns(["m1", "1.0", "m2", "2.0"])
    redis = build_redis
  ...
  def test_zrangestore
    @mock_client.expects(:zrangestore).with("dst", "z", 0, -1, byscore: false, bylex: false, rev: false, limit: nil).returns(3)
    redis = build_redis
    assert_equal 3, redis.zrangestore("dst", "z", 0, -1)
  end

  def test_zrangestore_with_by_score
    @mock_client.expects(:zrangestore).with("dst", "z", 0, 100, byscore: true, bylex: false, rev: false, limit: nil).returns(2)
    redis = build_redis
    assert_equal 2, redis.zrangestore("dst", "z", 0, 100, by_score: true)
  end

  def test_zrangestore_with_by_lex
    @mock_client.expects(:zrangestore).with("dst", "z", "[a", "[z", byscore: false, bylex: true, rev: false, limit: nil).returns(2)
    redis = build_redis
    assert_equal 2, redis.zrangestore("dst", "z", "[a", "[z", by_lex: true)
  end

  def test_zrevrange_without_scores
    @mock_client.expects(:zrevrange).with("z", 0, -1, withscores: false).returns(["m2", "m1"])
    redis = build_redis
    assert_equal ["m2", "m1"], redis.zrevrange("z", 0, -1)
  end
  ...
  def test_zrevrangebyscore
    @mock_client.expects(:zrevrangebyscore).with("z", "+inf", "-inf", withscores: false, limit: nil).returns(["m2", "m1"])
    redis = build_redis
    assert_equal ["m2", "m1"], redis.zrevrangebyscore("z", "+inf", "-inf")
  end

  def test_zrevrangebyscore_with_scores
    @mock_client.expects(:zrevrangebyscore).with("z", "+inf", "-inf", withscores: true, limit: nil).returns([["m2", "2.0"]])
    redis = build_redis
  ...
  def test_zincrby
    ...
    redis = build_redis
    assert_equal 5.0, redis.zincrby("z", 2, "m")
  end
  ...
  def test_zinterstore
    @mock_client.expects(:zinterstore).with("dst", ["z1", "z2"], weights: nil, aggregate: nil).returns(1)
    redis = build_redis
    assert_equal 1, redis.zinterstore("dst", ["z1", "z2"])
  end

  def test_zunionstore
    @mock_client.expects(:zunionstore).with("dst", ["z1", "z2"], weights: [1, 2], aggregate: "MAX").returns(3)
    redis = build_redis
    assert_equal 3, redis.zunionstore("dst", ["z1", "z2"], weights: [1, 2], aggregate: "MAX")
  end

  def test_zunion_without_scores
    @mock_client.expects(:zunion).with(["z1", "z2"], weights: nil, aggregate: nil, withscores: false).returns(["m1", "m2"])
    redis = build_redis
    assert_equal ["m1", "m2"], redis.zunion("z1", "z2")
  end
  ...
  def test_zinter_without_scores
    @mock_client.expects(:zinter).with(["z1", "z2"], weights: nil, aggregate: nil, withscores: false).returns(["m1"])
    redis = build_redis
  ...
  def test_zdiff_without_scores
    @mock_client.expects(:zdiff).with(["z1", "z2"], withscores: false).returns(["m1"])
    redis = build_redis
  ...
  def test_zdiffstore
    @mock_client.expects(:zdiffstore).with("dst", ["z1", "z2"]).returns(1)
    redis = build_redis
    assert_equal 1, redis.zdiffstore("dst", ["z1", "z2"])
  end
  ...
  def test_zrangebylex
    @mock_client.expects(:zrangebylex).with("z", "[a", "[z", limit: nil).returns(["a", "b"])
    redis = build_redis
    assert_equal ["a", "b"], redis.zrangebylex("z", "[a", "[z")
  end

  def test_zrevrangebylex
    @mock_client.expects(:zrevrangebylex).with("z", "[z", "[a", limit: nil).returns(["z", "b"])
    redis = build_redis
    assert_equal ["z", "b"], redis.zrevrangebylex("z", "[z", "[a")
  end
  ...
  def test_transform_scores_empty_array
    @mock_client.expects(:zrange).with("z", 0, -1, byscore: false, bylex: false, rev: false, limit: nil, withscores: true).returns([])
    redis = build_redis
    assert_equal [], redis.zrange("z", 0, -1, withscores: true)
  end
  ...
  def test_eval_with_keyword_keys_and_argv
    ...
    redis = build_redis
    assert_equal "OK", redis.eval("script", keys: ["k1", "k2"], argv: ["a1"])
  end
  ...
  def test_eval_with_2_positional_args
    ...
    redis = build_redis
    assert_equal "OK", redis.eval("script", ["k1", "k2"])
  end
  ...
  def test_evalsha_with_2_positional_args
    ...
    redis = build_redis
    assert_equal "OK", redis.evalsha("abc", ["k1", "k2"])
  end
  ...
  def test_script_exists_single_sha_returns_boolean
    ...
    redis = build_redis
    assert_equal true, redis.script("exists", "sha1")
  end
  ...
  def test_script_exists_array_returns_array
    ...
    redis = build_redis
    assert_equal [true, false], redis.script("exists", ["sha1", "sha2"])
  end
  ...
  def test_script_exists_direct
    @mock_client.expects(:script_exists).with("sha1").returns([1])
    redis = build_redis
    assert_equal true, redis.script_exists("sha1")
  end
  ...
  def test_bitop_array_flattened
    ...
    redis = build_redis
    assert_equal 4, redis.bitop("OR", "dst", ["k1", "k2"])
  end
  ...
  def test_lastsave
    @mock_client.expects(:lastsave).returns(1700000000)
    redis = build_redis
    assert_equal 1700000000, redis.lastsave
  end

  def test_time
    @mock_client.expects(:time).returns([1700000000, 123456])
    redis = build_redis
    assert_equal [1700000000, 123456], redis.time
  end
  ...
  def test_slowlog
    ...
    redis = build_redis
    assert_equal [], redis.slowlog("get", 10)
  end
  ...
  def test_pipelined_returns_results
    ...
    mock_pipeline.expects(:execute).returns(["OK", "v"])
  ...
  def test_multi_returns_results
    ...
    mock_transaction.expects(:execute).returns(["OK", "v"])
  ...
  def test_mapped_msetnx
    @mock_client.expects(:msetnx).with("k1", "v1").returns(1)
    redis = build_redis
    assert_equal true, redis.mapped_msetnx("k1" => "v1")
  end
  ...
  def test_scan_each_with_block
    enumerator = ["k1", "k2"].each
    @mock_client.expects(:scan_iter).with(match: "*", count: 10, type: nil).returns(enumerator)
    redis = build_redis
    collected = []
    redis.scan_each { |k| collected << k }
    assert_equal ["k1", "k2"], collected
  end
  ...
  def test_hscan_each_with_block
    enumerator = [["f1", "v1"]].each
    @mock_client.expects(:hscan_iter).with("h", match: "*", count: 10).returns(enumerator)
    redis = build_redis
    collected = []
    redis.hscan_each("h") { |pair| collected << pair }
    assert_equal [["f1", "v1"]], collected
  end
  ...
  def test_sscan_each_with_block
    enumerator = ["m1", "m2"].each
    @mock_client.expects(:sscan_iter).with("s", match: "*", count: 10).returns(enumerator)
    redis = build_redis
    collected = []
    redis.sscan_each("s") { |m| collected << m }
    assert_equal ["m1", "m2"], collected
  end
  ...
  def test_expire_question_mark
    @mock_client.expects(:expire).with("k", 60, nx: false, xx: false, gt: false, lt: false).returns(1)
    redis = build_redis
    assert_equal true, redis.expire?("k", 60)
  end

  def test_expireat_question_mark
    @mock_client.expects(:expireat).with("k", 1700000000, nx: false, xx: false, gt: false, lt: false).returns(1)
    redis = build_redis
    assert_equal true, redis.expireat?("k", 1700000000)
  end

  def test_persist_question_mark
    @mock_client.expects(:persist).with("k").returns(1)
    redis = build_redis
    assert_equal true, redis.persist?("k")
  end

  def test_renamenx_question_mark
    @mock_client.expects(:renamenx).with("old", "new").returns(1)
    redis = build_redis
    assert_equal true, redis.renamenx?("old", "new")
  end
  ...
  def test_hincrbyfloat_compat_with_string
    @mock_client.expects(:hincrbyfloat).with("h", "f", 1.5).returns("3.5")
    redis = build_redis
    assert_equal 3.5, redis.hincrbyfloat_compat("h", "f", 1.5)
  end
  ...
  def test_zincrby_compat_with_string
    @mock_client.expects(:zincrby).with("z", 2, "m").returns("5.0")
    redis = build_redis
    assert_equal 5.0, redis.zincrby_compat("z", 2, "m")
  end
  ...
  def test_future_command
    future = Redis::Future.new(["SET", "k", "v"])
  ...
  def test_future_instance_of
    ...
    refute future.instance_of?(Object)
  end

  def test_future_kind_of
    future = Redis::Future.new(["GET", "k"])
    assert future.kind_of?(Redis::Future)
  end
  ...
  def test_future_instance_variable_get
    future = Redis::Future.new(["GET", "k"])
    assert_equal ["GET", "k"], future.instance_variable_get(:@command)
  ...
  def test_default_options_frozen
    assert Redis::DEFAULT_OPTIONS.frozen?
  end
  ...
  def test_default_options_values
    ...
    assert_equal 5.0, Redis::DEFAULT_OPTIONS[:timeout]
  end
