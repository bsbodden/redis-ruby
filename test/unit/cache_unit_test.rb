...
class CacheUnitTest < Minitest::Test
  ...
  def test_enable_default_mode
    ...
    assert_equal ["CLIENT", "TRACKING", "ON"], @mock_client.call_history.first
  end

  def test_enable_optin_mode
    cache = RedisRuby::Cache.new(@mock_client, mode: :optin)
    cache.enable!
    assert_equal ["CLIENT", "TRACKING", "ON", "OPTIN"], @mock_client.call_history.first
  end

  def test_enable_optout_mode
    cache = RedisRuby::Cache.new(@mock_client, mode: :optout)
    cache.enable!
    assert_equal ["CLIENT", "TRACKING", "ON", "OPTOUT"], @mock_client.call_history.first
  end

  def test_enable_broadcast_mode
    cache = RedisRuby::Cache.new(@mock_client, mode: :broadcast)
    cache.enable!
    assert_equal ["CLIENT", "TRACKING", "ON", "BCAST"], @mock_client.call_history.first
  end
  ...
  def test_get_optin_sends_caching_yes_command
    ...
    caching_calls = @mock_client.call_history.select { |c| c == ["CLIENT", "CACHING", "YES"] }
  ...
  def test_invalidate_all_multiple_keys
    ...
    count = cache.invalidate_all(["key1", "key2"])
  ...
  def test_invalidate_all_mixed_existing_and_missing
    ...
    count = cache.invalidate_all(["key1", "nonexistent"])
  ...
  def test_process_invalidation_with_multiple_keys
    ...
    result = cache.process_invalidation(["invalidate", ["key1", "key2"]])
    assert result
    refute cache.cached?("key1")
    refute cache.cached?("key2")
  end
