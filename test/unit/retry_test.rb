...
class RetryPolicyTest < Minitest::Test
  ...
  def test_retry_retries_on_connection_error
    policy = RR::Retry.new(retries: 3, backoff: RR::NoBackoff.new)
    calls = 0
    result = policy.call do
      calls += 1
      raise RR::ConnectionError, "lost" if calls < 3
  ...
  def test_retry_retries_on_timeout_error
    ...
    result = policy.call do
      calls += 1
      raise RR::TimeoutError, "timed out" if calls < 2
  ...
  def test_retry_raises_after_exhausting_retries
    policy = RR::Retry.new(retries: 2, backoff: RR::NoBackoff.new)
    calls = 0
    assert_raises(RR::ConnectionError) do
      policy.call do
        calls += 1
        raise RR::ConnectionError, "always fails"
      end
    end
  ...
  def test_retry_does_not_catch_command_errors
    ...
    assert_raises(RR::CommandError) do
      policy.call do
        calls += 1
        raise RR::CommandError, "ERR wrong type"
      end
    end
    assert_equal 1, calls
  end
  ...
  def test_retry_with_custom_supported_errors
    policy = RR::Retry.new(
      retries: 2,
      supported_errors: [RR::ConnectionError],
      backoff: RR::NoBackoff.new
    )
  ...
  def test_retry_default_supported_errors
    ...
    result = policy.call do
      calls += 1
      raise RR::ConnectionError, "lost" if calls == 1
      raise RR::TimeoutError, "slow" if calls == 2
  ...
  def test_retry_zero_retries_raises_immediately
    policy = RR::Retry.new(retries: 0, backoff: RR::NoBackoff.new)
    calls = 0
    assert_raises(RR::ConnectionError) do
      policy.call do
        calls += 1
