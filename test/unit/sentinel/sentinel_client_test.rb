...
class SentinelClientTest < Minitest::Test
  ...
  def test_readonly_error_detection
    ...
    error1 = RR::CommandError.new("READONLY You can't write against a read only replica")

    assert client.send(:readonly_error?, error1)

    error2 = RR::CommandError.new("ERR wrong number of arguments")
  ...
  def test_timeout_default
    client = RR::SentinelClient.allocate
    client.instance_variable_set(:@timeout, RR::SentinelClient::DEFAULT_TIMEOUT)
