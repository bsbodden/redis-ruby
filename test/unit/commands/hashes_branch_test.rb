...
class HashesBranchTest < Minitest::Test
  class MockClient
    ...
    def mock_return(args)
      case args[0]
      when "HGETALL" then ["f1", "v1", "f2", "v2"]
      when "HSCAN" then ["0", ["f1", "v1", "f2", "v2"]]
      when "HRANDFIELD"
  ...
  def test_hset_single_field_fast_path
    ...
    assert_equal ["HSET", "key", "f1", "v1"], @client.last_command
  end

  def test_hset_multiple_fields
    @client.hset("key", "f1", "v1", "f2", "v2")
    assert_equal ["HSET", "key", "f1", "v1", "f2", "v2"], @client.last_command
  end

  # hget, hsetnx
  def test_hget
    @client.hget("key", "field")
    assert_equal ["HGET", "key", "field"], @client.last_command
  end

  def test_hsetnx
    @client.hsetnx("key", "field", "val")
    assert_equal ["HSETNX", "key", "field", "val"], @client.last_command
  end

  # hmget, hmset
  def test_hmget
    @client.hmget("key", "f1", "f2")
    assert_equal ["HMGET", "key", "f1", "f2"], @client.last_command
  end

  def test_hmset
    @client.hmset("key", "f1", "v1", "f2", "v2")
    assert_equal ["HMSET", "key", "f1", "v1", "f2", "v2"], @client.last_command
  end
  ...
  def test_hdel_single_field_fast_path
    @client.hdel("key", "f1")
    assert_equal ["HDEL", "key", "f1"], @client.last_command
  end

  def test_hdel_multiple_fields
    @client.hdel("key", "f1", "f2", "f3")
    assert_equal ["HDEL", "key", "f1", "f2", "f3"], @client.last_command
  end

  # hexists, hkeys, hvals, hlen, hstrlen
  def test_hexists
    @client.hexists("key", "field")
    assert_equal ["HEXISTS", "key", "field"], @client.last_command
  end

  def test_hkeys
    @client.hkeys("key")
    assert_equal ["HKEYS", "key"], @client.last_command
  end

  def test_hvals
    @client.hvals("key")
    assert_equal ["HVALS", "key"], @client.last_command
  end

  def test_hlen
    @client.hlen("key")
    assert_equal ["HLEN", "key"], @client.last_command
  end

  def test_hstrlen
    @client.hstrlen("key", "field")
    assert_equal ["HSTRLEN", "key", "field"], @client.last_command
  end
  ...
  def test_hscan_no_options
    ...
    assert_equal [["f1", "v1"], ["f2", "v2"]], pairs
  end
  ...
  def test_hpexpire
    @client.hpexpire("key", 60000, "f1")
    assert_equal "HPEXPIRE", @client.last_command[0]
  end

  def test_hexpireat
    @client.hexpireat("key", 1700000000, "f1")
    assert_equal "HEXPIREAT", @client.last_command[0]
  end

  def test_hpexpireat
    @client.hpexpireat("key", 1700000000000, "f1")
