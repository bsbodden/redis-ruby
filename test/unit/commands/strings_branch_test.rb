...
class StringsBranchTest < Minitest::Test
  ...
  def test_set_simple_fast_path
    ...
    assert_equal %w[SET mykey myval], @client.last_command
  end
  ...
  def test_set_with_px
    @client.set("mykey", "myval", px: 60_000)

    assert_equal ["SET", "mykey", "myval", "PX", 60_000], @client.last_command
  end

  def test_set_with_exat
    @client.set("mykey", "myval", exat: 1_700_000_000)

    assert_equal ["SET", "mykey", "myval", "EXAT", 1_700_000_000], @client.last_command
  end

  def test_set_with_pxat
    @client.set("mykey", "myval", pxat: 1_700_000_000_000)

    assert_equal ["SET", "mykey", "myval", "PXAT", 1_700_000_000_000], @client.last_command
  end

  def test_set_with_nx
    @client.set("mykey", "myval", nx: true)

    assert_equal %w[SET mykey myval NX], @client.last_command
  end

  def test_set_with_xx
    @client.set("mykey", "myval", xx: true)

    assert_equal %w[SET mykey myval XX], @client.last_command
  end

  def test_set_with_keepttl
    @client.set("mykey", "myval", keepttl: true)

    assert_equal %w[SET mykey myval KEEPTTL], @client.last_command
  end

  def test_set_with_get
    @client.set("mykey", "myval", get: true)

    assert_equal %w[SET mykey myval GET], @client.last_command
  end
  ...
  def test_set_with_px_and_xx
    @client.set("mykey", "myval", px: 60_000, xx: true)

    assert_equal ["SET", "mykey", "myval", "PX", 60_000, "XX"], @client.last_command
  end

  def test_set_with_exat_and_keepttl_and_get
    @client.set("mykey", "myval", exat: 1_700_000_000, keepttl: true, get: true)

    assert_equal ["SET", "mykey", "myval", "EXAT", 1_700_000_000, "KEEPTTL", "GET"], @client.last_command
  end

  def test_set_all_options
    @client.set("mykey", "myval", ex: 60, px: 1000, exat: 170, pxat: 170_000, nx: true, xx: true, keepttl: true,
                                  get: true)
    expected = ["SET", "mykey", "myval", "EX", 60, "PX", 1000, "EXAT", 170, "PXAT", 170_000, "NX", "XX", "KEEPTTL",
                "GET",]
  ...
  def test_get
    ...
    assert_equal %w[GET mykey], @client.last_command
  ...
  def test_incr
    ...
    assert_equal %w[INCR counter], @client.last_command
  ...
  def test_decr
    ...
    assert_equal %w[DECR counter], @client.last_command
    assert_equal 42, result
  end
  ...
  def test_append
    ...
    assert_equal %w[APPEND mykey extra], @client.last_command
    assert_equal 42, result
  end
  ...
  def test_strlen
    ...
    assert_equal %w[STRLEN mykey], @client.last_command
    assert_equal 42, result
  end
  ...
  def test_mget
    ...
    assert_equal %w[MGET key1 key2], @client.last_command
  ...
  def test_mget_single_key
    ...
    assert_equal %w[MGET key1], @client.last_command
  end

  def test_mset
    @client.mset("k1", "v1", "k2", "v2")

    assert_equal %w[MSET k1 v1 k2 v2], @client.last_command
  end

  def test_msetnx
    result = @client.msetnx("k1", "v1", "k2", "v2")

    assert_equal %w[MSETNX k1 v1 k2 v2], @client.last_command
  ...
  def test_setnx_returns_true_when_set
    ...
    assert_equal %w[SETNX mykey myval], @client.last_command
    assert result
  end
  ...
  def test_psetex
    result = @client.psetex("mykey", 60_000, "myval")

    assert_equal ["PSETEX", "mykey", 60_000, "myval"], @client.last_command
    assert_equal "OK", result
  end
  ...
  def test_getset
    ...
    assert_equal %w[GETSET mykey newval], @client.last_command
    assert_equal "value", result
  end
  ...
  def test_getdel
    ...
    assert_equal %w[GETDEL mykey], @client.last_command
    assert_equal "value", result
  end
  ...
  def test_getex_no_options
    ...
    assert_equal %w[GETEX mykey], @client.last_command
    assert_equal "value", result
  end
  ...
  def test_getex_with_px
    @client.getex("mykey", px: 60_000)

    assert_equal ["GETEX", "mykey", "PX", 60_000], @client.last_command
  end

  def test_getex_with_exat
    @client.getex("mykey", exat: 1_700_000_000)

    assert_equal ["GETEX", "mykey", "EXAT", 1_700_000_000], @client.last_command
  end

  def test_getex_with_pxat
    @client.getex("mykey", pxat: 1_700_000_000_000)

    assert_equal ["GETEX", "mykey", "PXAT", 1_700_000_000_000], @client.last_command
  end

  def test_getex_with_persist
    @client.getex("mykey", persist: true)

    assert_equal %w[GETEX mykey PERSIST], @client.last_command
  end
  ...
  def test_getex_all_expiry_options
    @client.getex("mykey", ex: 60, px: 1000, exat: 170, pxat: 170_000, persist: true)

    assert_equal ["GETEX", "mykey", "EX", 60, "PX", 1000, "EXAT", 170, "PXAT", 170_000, "PERSIST"],
                 @client.last_command
  end
  ...
  def test_set_ex_nil_does_not_add
    ...
    # nil ex => fast path
    assert_equal %w[SET k v], @client.last_command
  end
  ...
  def test_getex_ex_nil_does_not_add
    ...
    assert_equal %w[GETEX k], @client.last_command
  end
