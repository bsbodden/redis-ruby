...
class ServerBranchTest < Minitest::Test
  ...
  def test_info_with_section
    @client.info("server")
    assert_equal ["INFO", "server"], @client.last_command
  end
  ...
  def test_flushdb_async
    ...
    assert_equal ["FLUSHDB", "ASYNC"], @client.last_command
  end

  def test_flushdb_sync
    @client.flushdb(:sync)
    assert_equal ["FLUSHDB", "SYNC"], @client.last_command
  end
  ...
  def test_flushall_async
    ...
    assert_equal ["FLUSHALL", "ASYNC"], @client.last_command
  end

  def test_flushall_sync
    @client.flushall(:sync)
    assert_equal ["FLUSHALL", "SYNC"], @client.last_command
  end
  ...
  def test_bgsave_with_schedule
    ...
    assert_equal ["BGSAVE", "SCHEDULE"], @client.last_command
  end
  ...
  def test_shutdown_nosave
    ...
    assert_equal ["SHUTDOWN", "NOSAVE"], @client.last_command
  end

  def test_shutdown_save
    @client.shutdown(:save)
    assert_equal ["SHUTDOWN", "SAVE"], @client.last_command
  end
  ...
  def test_config_get
    ...
    assert_equal ["CONFIG", "GET", "maxmemory"], @client.last_command
  end

  def test_config_set
    @client.config_set("maxmemory", "100mb")
    assert_equal ["CONFIG", "SET", "maxmemory", "100mb"], @client.last_command
  end

  def test_config_rewrite
    @client.config_rewrite
    assert_equal ["CONFIG", "REWRITE"], @client.last_command
  end

  def test_config_resetstat
    @client.config_resetstat
    assert_equal ["CONFIG", "RESETSTAT"], @client.last_command
  end

  # client
  def test_client_list_no_type
    @client.client_list
    assert_equal ["CLIENT", "LIST"], @client.last_command
  end

  def test_client_list_with_type
    @client.client_list(type: "normal")
    assert_equal ["CLIENT", "LIST", "TYPE", "normal"], @client.last_command
  end

  def test_client_getname
    @client.client_getname
    assert_equal ["CLIENT", "GETNAME"], @client.last_command
  end

  def test_client_setname
    @client.client_setname("myconn")
    assert_equal ["CLIENT", "SETNAME", "myconn"], @client.last_command
  end

  def test_client_id
    @client.client_id
    assert_equal ["CLIENT", "ID"], @client.last_command
  end

  def test_client_info
    @client.client_info
    assert_equal ["CLIENT", "INFO"], @client.last_command
  end
  ...
  def test_client_kill_no_filter
    @client.client_kill
    assert_equal ["CLIENT", "KILL"], @client.last_command
  end
  ...
  def test_client_unpause
    ...
    assert_equal ["CLIENT", "UNPAUSE"], @client.last_command
  end

  def test_client_no_evict_enabled
    @client.client_no_evict(true)
    assert_equal ["CLIENT", "NO-EVICT", "ON"], @client.last_command
  end

  def test_client_no_evict_disabled
    @client.client_no_evict(false)
    assert_equal ["CLIENT", "NO-EVICT", "OFF"], @client.last_command
  end

  # slowlog
  def test_slowlog_get_no_count
    @client.slowlog_get
    assert_equal ["SLOWLOG", "GET"], @client.last_command
  end
  ...
  def test_slowlog_len
    @client.slowlog_len
    assert_equal ["SLOWLOG", "LEN"], @client.last_command
  end

  def test_slowlog_reset
    @client.slowlog_reset
    assert_equal ["SLOWLOG", "RESET"], @client.last_command
  end

  # memory
  def test_memory_doctor
    @client.memory_doctor
    assert_equal ["MEMORY", "DOCTOR"], @client.last_command
  end

  def test_memory_stats
    @client.memory_stats
    assert_equal ["MEMORY", "STATS"], @client.last_command
  end

  def test_memory_purge
    @client.memory_purge
    assert_equal ["MEMORY", "PURGE"], @client.last_command
  end

  def test_memory_malloc_stats
    @client.memory_malloc_stats
    assert_equal ["MEMORY", "MALLOC-STATS"], @client.last_command
  end

  # object
  def test_object_encoding
    @client.object_encoding("key")
    assert_equal ["OBJECT", "ENCODING", "key"], @client.last_command
  end

  def test_object_freq
    @client.object_freq("key")
    assert_equal ["OBJECT", "FREQ", "key"], @client.last_command
  end

  def test_object_idletime
    @client.object_idletime("key")
    assert_equal ["OBJECT", "IDLETIME", "key"], @client.last_command
  end

  def test_object_refcount
    @client.object_refcount("key")
    assert_equal ["OBJECT", "REFCOUNT", "key"], @client.last_command
  end

  # command
  def test_command_count
    @client.command_count
    assert_equal ["COMMAND", "COUNT"], @client.last_command
  end

  def test_command_docs_single
    @client.command_docs("GET")
    assert_equal ["COMMAND", "DOCS", "GET"], @client.last_command
  end

  def test_command_docs_multiple
    @client.command_docs("GET", "SET")
    assert_equal ["COMMAND", "DOCS", "GET", "SET"], @client.last_command
  end

  def test_command_info_single
    @client.command_info("GET")
    assert_equal ["COMMAND", "INFO", "GET"], @client.last_command
  end

  def test_command_info_multiple
    @client.command_info("GET", "SET")
    assert_equal ["COMMAND", "INFO", "GET", "SET"], @client.last_command
  end

  def test_command_list
    @client.command_list
    assert_equal ["COMMAND", "LIST"], @client.last_command
  end

  # latency
  def test_latency_latest
    @client.latency_latest
    assert_equal ["LATENCY", "LATEST"], @client.last_command
  end

  def test_latency_history
    @client.latency_history("command")
    assert_equal ["LATENCY", "HISTORY", "command"], @client.last_command
  end

  def test_latency_reset_no_events
    @client.latency_reset
    assert_equal ["LATENCY", "RESET"], @client.last_command
  end

  def test_latency_reset_single_event
    @client.latency_reset("command")
    assert_equal ["LATENCY", "RESET", "command"], @client.last_command
  end
  ...
  def test_module_list
    @client.module_list
    assert_equal ["MODULE", "LIST"], @client.last_command
  end
  ...
  def test_module_unload
    ...
    assert_equal ["MODULE", "UNLOAD", "mymodule"], @client.last_command
  end
  ...
  def test_replicaof_no_one
    ...
    assert_equal ["REPLICAOF", "NO", "ONE"], @client.last_command
  end
  ...
  def test_echo
    @client.echo("hello")
    assert_equal ["ECHO", "hello"], @client.last_command
  end

  def test_debug_object
    @client.debug_object("key")
    assert_equal ["DEBUG", "OBJECT", "key"], @client.last_command
  end
