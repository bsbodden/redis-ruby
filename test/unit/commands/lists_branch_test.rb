...
class ListsBranchTest < Minitest::Test
  class MockClient
    include RedisRuby::Commands::Lists
    attr_reader :last_command

    def call(*args) = (@last_command = args; "OK")
    def call_1arg(cmd, a1) = (@last_command = [cmd, a1]; "OK")
    def call_2args(cmd, a1, a2) = (@last_command = [cmd, a1, a2]; 1)
    def call_3args(cmd, a1, a2, a3) = (@last_command = [cmd, a1, a2, a3]; "OK")
  end
  ...
  def test_lpush_single_value_fast_path
    ...
    assert_equal ["LPUSH", "key", "val"], @client.last_command
  end

  def test_lpush_multiple_values
    @client.lpush("key", "v1", "v2", "v3")
    assert_equal ["LPUSH", "key", "v1", "v2", "v3"], @client.last_command
  end

  # lpushx
  def test_lpushx_single_value_fast_path
    @client.lpushx("key", "val")
    assert_equal ["LPUSHX", "key", "val"], @client.last_command
  end

  def test_lpushx_multiple_values
    @client.lpushx("key", "v1", "v2")
    assert_equal ["LPUSHX", "key", "v1", "v2"], @client.last_command
  end

  # rpush
  def test_rpush_single_value_fast_path
    @client.rpush("key", "val")
    assert_equal ["RPUSH", "key", "val"], @client.last_command
  end

  def test_rpush_multiple_values
    @client.rpush("key", "v1", "v2")
    assert_equal ["RPUSH", "key", "v1", "v2"], @client.last_command
  end

  # rpushx
  def test_rpushx_single_value_fast_path
    @client.rpushx("key", "val")
    assert_equal ["RPUSHX", "key", "val"], @client.last_command
  end

  def test_rpushx_multiple_values
    @client.rpushx("key", "v1", "v2")
    assert_equal ["RPUSHX", "key", "v1", "v2"], @client.last_command
  end

  # lpop
  def test_lpop_no_count
    @client.lpop("key")
    assert_equal ["LPOP", "key"], @client.last_command
  end
  ...
  def test_rpop_no_count
    @client.rpop("key")
    assert_equal ["RPOP", "key"], @client.last_command
  end
  ...
  def test_llen
    @client.llen("key")
    assert_equal ["LLEN", "key"], @client.last_command
  end
  ...
  def test_linsert_before
    ...
    assert_equal ["LINSERT", "key", "BEFORE", "pivot", "val"], @client.last_command
  end

  def test_linsert_after
    @client.linsert("key", :after, "pivot", "val")
    assert_equal ["LINSERT", "key", "AFTER", "pivot", "val"], @client.last_command
  end
  ...
  def test_rpoplpush
    ...
    assert_equal ["RPOPLPUSH", "src", "dst"], @client.last_command
  end

  def test_lmove
    @client.lmove("src", "dst", :left, :right)
    assert_equal ["LMOVE", "src", "dst", "LEFT", "RIGHT"], @client.last_command
  end
  ...
  def test_lpos_fast_path
    ...
    assert_equal ["LPOS", "key", "elem"], @client.last_command
  end
