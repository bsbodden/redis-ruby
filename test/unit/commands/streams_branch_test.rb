...
class StreamsBranchTest < Minitest::Test
  class MockClient
    include RedisRuby::Commands::Streams
    ...
    def mock_return(args)
      case args[0]
      when "XRANGE", "XREVRANGE" then [["id1", ["f1", "v1"]], ["id2", ["f2", "v2"]]]
      when "XREAD", "XREADGROUP" then [["stream1", [["id1", ["f1", "v1"]]]]]
      when "XPENDING" then ["summary"]
      when "XCLAIM" then [["id1", ["f1", "v1"]]]
      when "XAUTOCLAIM" then ["next-id", [["id1", ["f1", "v1"]]], []]
      when "XINFO"
        if args[1] == "STREAM" then ["key1", "val1", "key2", "val2"]
        elsif args[1] == "GROUPS" then [["name", "g1", "consumers", "1"]]
        elsif args[1] == "CONSUMERS" then [["name", "c1", "pending", "0"]]
        end
  ...
  def test_xadd_with_custom_id
    ...
    assert_equal ["XADD", "mystream", "1000-0", "f", "v"], @client.last_command
  end
  ...
  def test_xlen
    ...
    assert_equal ["XLEN", "mystream"], @client.last_command
  ...
  def test_xread_single_stream_with_id
    ...
    assert_equal ["XREAD", "STREAMS", "mystream", "0-0"], @client.last_command
  ...
  def test_xgroup_destroy
    ...
    assert_equal ["XGROUP", "DESTROY", "mystream", "mygroup"], @client.last_command
    assert_equal "OK", result
  end
  ...
  def test_xgroup_setid
    ...
    assert_equal ["XGROUP", "SETID", "mystream", "mygroup", "0-0"], @client.last_command
    assert_equal "OK", result
  end
  ...
  def test_xgroup_createconsumer
    ...
    assert_equal ["XGROUP", "CREATECONSUMER", "mystream", "mygroup", "consumer1"], @client.last_command
    assert_equal "OK", result
  end
  ...
  def test_xgroup_delconsumer
    ...
    assert_equal ["XGROUP", "DELCONSUMER", "mystream", "mygroup", "consumer1"], @client.last_command
    assert_equal "OK", result
  end
  ...
  def test_xack_single_id_fast_path
    ...
    assert_equal ["XACK", "mystream", "mygroup", "1000-0"], @client.last_command
    assert_equal 1, result
  end

  def test_xack_multiple_ids
    result = @client.xack("mystream", "mygroup", "1000-0", "1001-0", "1002-0")
    assert_equal ["XACK", "mystream", "mygroup", "1000-0", "1001-0", "1002-0"], @client.last_command
    assert_equal 1, result
  end
  ...
  def test_xpending_summary_no_range_args
    ...
    assert_equal ["XPENDING", "mystream", "mygroup"], @client.last_command
  ...
  def test_xclaim_basic
    result = @client.xclaim("mystream", "mygroup", "consumer1", 60000, "1000-0")
    assert_equal ["XCLAIM", "mystream", "mygroup", "consumer1", 60000, "1000-0"], @client.last_command
    # parse_entries is called when justid is false
    assert_equal [["id1", { "f1" => "v1" }]], result
  end

  def test_xclaim_multiple_ids
    result = @client.xclaim("mystream", "mygroup", "consumer1", 60000, "1000-0", "1001-0")
    assert_equal ["XCLAIM", "mystream", "mygroup", "consumer1", 60000, "1000-0", "1001-0"], @client.last_command
    assert_equal [["id1", { "f1" => "v1" }]], result
  end

  def test_xclaim_with_idle
    @client.xclaim("mystream", "mygroup", "consumer1", 60000, "1000-0", idle: 120000)
    assert_equal(
      ["XCLAIM", "mystream", "mygroup", "consumer1", 60000, "1000-0", "IDLE", 120000],
      @client.last_command
    )
  end

  def test_xclaim_with_time
    @client.xclaim("mystream", "mygroup", "consumer1", 60000, "1000-0", time: 1609459200000)
    assert_equal(
      ["XCLAIM", "mystream", "mygroup", "consumer1", 60000, "1000-0", "TIME", 1609459200000],
      @client.last_command
    )
  end

  def test_xclaim_with_retrycount
    @client.xclaim("mystream", "mygroup", "consumer1", 60000, "1000-0", retrycount: 3)
    assert_equal(
      ["XCLAIM", "mystream", "mygroup", "consumer1", 60000, "1000-0", "RETRYCOUNT", 3],
      @client.last_command
    )
  end

  def test_xclaim_with_force
    @client.xclaim("mystream", "mygroup", "consumer1", 60000, "1000-0", force: true)
    assert_equal(
      ["XCLAIM", "mystream", "mygroup", "consumer1", 60000, "1000-0", "FORCE"],
      @client.last_command
    )
  end

  def test_xclaim_without_force
    @client.xclaim("mystream", "mygroup", "consumer1", 60000, "1000-0", force: false)
  ...
  def test_xclaim_with_justid
    ...
    nil_client = NilReturningMockClient.new
    result = nil_client.xclaim("mystream", "mygroup", "consumer1", 60000, "1000-0", justid: true)
  ...
  def test_xclaim_justid_returns_result_directly
    ...
    result = client.xclaim("mystream", "mygroup", "consumer1", 60000, "1000-0", justid: true)
  ...
  def test_xclaim_without_justid_parses_entries
    result = @client.xclaim("mystream", "mygroup", "consumer1", 60000, "1000-0", justid: false)
    # Without justid, parse_entries converts [id, [f, v]] -> [id, {f => v}]
    assert_equal [["id1", { "f1" => "v1" }]], result
  end

  def test_xclaim_with_all_options
    @client.xclaim("mystream", "mygroup", "consumer1", 60000, "1000-0",
                   idle: 120000, time: 1609459200000, retrycount: 5, force: true, justid: true)
    assert_equal(
      ["XCLAIM", "mystream", "mygroup", "consumer1", 60000, "1000-0",
       "IDLE", 120000, "TIME", 1609459200000, "RETRYCOUNT", 5, "FORCE", "JUSTID"],
      @client.last_command
    )
  end
  ...
  def test_xautoclaim_basic
    result = @client.xautoclaim("mystream", "mygroup", "consumer1", 60000, "0-0")
    assert_equal ["XAUTOCLAIM", "mystream", "mygroup", "consumer1", 60000, "0-0"], @client.last_command
  ...
  def test_xautoclaim_with_count
    @client.xautoclaim("mystream", "mygroup", "consumer1", 60000, "0-0", count: 10)
    assert_equal(
      ["XAUTOCLAIM", "mystream", "mygroup", "consumer1", 60000, "0-0", "COUNT", 10],
      @client.last_command
    )
  end

  def test_xautoclaim_without_count
    @client.xautoclaim("mystream", "mygroup", "consumer1", 60000, "0-0")
    refute_includes @client.last_command, "COUNT"
  end

  def test_xautoclaim_with_justid
    result = @client.xautoclaim("mystream", "mygroup", "consumer1", 60000, "0-0", justid: true)
  ...
  def test_xautoclaim_without_justid
    result = @client.xautoclaim("mystream", "mygroup", "consumer1", 60000, "0-0", justid: false)
    refute_includes @client.last_command, "JUSTID"
  ...
  def test_xautoclaim_with_count_and_justid
    @client.xautoclaim("mystream", "mygroup", "consumer1", 60000, "0-0", count: 5, justid: true)
    assert_equal(
      ["XAUTOCLAIM", "mystream", "mygroup", "consumer1", 60000, "0-0", "COUNT", 5, "JUSTID"],
      @client.last_command
    )
  end
  ...
  def test_xautoclaim_nil_result
    nil_client = NilReturningMockClient.new
    result = nil_client.xautoclaim("mystream", "mygroup", "consumer1", 60000, "0-0")
    assert_nil result
  end
  ...
  def test_xinfo_stream_basic
    ...
    assert_equal ["XINFO", "STREAM", "mystream"], @client.last_command
  ...
  def test_xinfo_stream_with_full
    ...
    assert_equal ["XINFO", "STREAM", "mystream", "FULL"], @client.last_command
    assert_equal({ "key1" => "val1", "key2" => "val2" }, result)
  end
  ...
  def test_xinfo_groups
    ...
    assert_equal ["XINFO", "GROUPS", "mystream"], @client.last_command
  ...
  def test_xinfo_consumers
    ...
    assert_equal ["XINFO", "CONSUMERS", "mystream", "mygroup"], @client.last_command
  ...
  def test_xdel_single_id_fast_path
    ...
    assert_equal ["XDEL", "mystream", "1000-0"], @client.last_command
    assert_equal 1, result
  end

  def test_xdel_multiple_ids
    result = @client.xdel("mystream", "1000-0", "1001-0", "1002-0")
    assert_equal ["XDEL", "mystream", "1000-0", "1001-0", "1002-0"], @client.last_command
    assert_equal 1, result
  end
