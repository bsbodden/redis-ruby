...
class StatsDExporterTest < Minitest::Test
  def setup
    ...
    @port = 18_125 # Use non-standard port for testing
  ...
  def test_export_sends_metrics
    ...
    assert_predicate @received_metrics.size, :positive?, "Should have received metrics"
    ...
    assert_match(/test\.redis\.command\.count:\d+\|c\|#(command:SET,environment:test|environment:test,command:SET)/,
                 metrics_text, "Should have SET command counter with tags")
  end
  ...
  def test_export_returns_metric_count
    ...
    @exporter.stub(:send_metric, nil) do
      ...
      assert_predicate count, :positive?, "Should return number of metrics sent"
    end
  end
  ...
  def test_close_closes_socket
    ...
    refute_predicate socket, :closed?, "Socket should be open initially"

    @exporter.close

    assert_predicate socket, :closed?, "Socket should be closed after close()"
  end
  ...
  def test_export_with_pool_metrics
    ...
    @exporter.stub(:send_metric, nil) do
      ...
      assert_predicate count, :positive?, "Should export pool metrics"
    end
  end
  ...
  def test_export_with_error_metrics
    ...
    @exporter.stub(:send_metric, nil) do
      ...
      assert_predicate count, :positive?, "Should export error metrics"
    end
  end
  ...
  def test_export_with_pipeline_metrics
    ...
    @exporter.stub(:send_metric, nil) do
      ...
      assert_predicate count, :positive?, "Should export pipeline metrics"
    end
  end
end
