...
redis.time_series("requests:raw") do
  ...
  compact_to "requests:hourly", :avg, 3600000 do
    retention 86400000
    labels resolution: "hourly"
  end
  
  compact_to "requests:daily", :avg, 86400000 do
...
now = Time.now.to_i * 1000
10.times do |i|
  redis.ts_add("temperature:room1", now + (i * 60000), 20 + rand(5))
end
...
result = redis.ts_range("temperature:room1", "-", "+",
  aggregation: "avg", bucket_duration: 300000)
